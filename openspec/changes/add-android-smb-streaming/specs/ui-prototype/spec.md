...
#### Scenario: SMB profile editor accepts both formats
- **WHEN** the user creates or edits an SMB profile
- **THEN** the UI SHALL accept either `\\host\\share` or `smb://host/share` as the profile root input
- **AND** the UI SHALL provide credential fields and a "remember password" option
